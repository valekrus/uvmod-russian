...
customFileInput.addEventListener('change', function () {
    ...
    if (this.files.length > 0) {
        ...
        customFileLabel.textContent = 'Select own firmware (v27 only)';
    }
});
...
document.getElementById('patchButton').addEventListener('click', function () {
    ...
    const file = useDefaultFirmwareCheckbox.checked
        ? fetch('fw/k5_v2.01.27_flashable.bin')
            .then((response) => {
                if (!response.ok) {
